...
class DiffusionTrainingModule(LightningModule):
    ...
    def training_step(self, batch: t.Tensor, batch_idx: int) -> t.Tensor:
        ...
        if self.__skip_one_step:
            ...
            self.log('train_mse', losses.mse_loss, on_step=True, prog_bar=True)
            self.log('train_t0_loss', losses.t0_loss, on_step=True, prog_bar=True)
            self.log('train_tT_loss', losses.tT_loss, on_step=True, prog_bar=True)
            self.log('train_decoder_nll', losses.decoder_nll, on_step=True, prog_bar=True)
            self.log('train_perplexity', t.exp(losses.decoder_nll.mean()), on_step=True, prog_bar=True)
        
        return L2Wrap.apply(losses.loss, y.to(self.dtype))
    ...
    def validation_step(self, batch: t.Tensor, batch_idx: int) -> t.Tensor:
        ...
        if not self.trainer.sanity_checking and not self.skip_validation_step:
            ...
            self.log('validation_loss', losses.loss)
            self.log('validation_mse', losses.mse_loss)
            self.log('validation_t0_loss', losses.t0_loss)
            self.log('validation_tT_loss', losses.tT_loss)
            self.log('validation_decoder_nll', losses.decoder_nll)
    ...
    def _process_batch(self, batch: t.Tensor):
        _, x, y = batch
        y = y.to(t.int64)
        diff_out: DiffusionLosses = self.model(x, y)
